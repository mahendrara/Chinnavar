...
@Named("plannedTripController")
@SessionScoped
public class PlannedTripController implements Serializable {

    private PlannedTrip current;
    private DataModel items = null;
    @Inject
    private schedule.sessions.PlannedTripFacade ejbFacade;
    @Inject
...
//        }
    ...
    public void setSelectedStartTTObject() {
        recreateModel();
    }
    ...
    public PlannedTrip getSelected() {
        if (current == null) {
            current = new PlannedTrip();
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public StreamedContent getTrainGraph() {
        //Chart
        try {
            PlannedTrip tripToDraw = getSelected();
    ...
    private PlannedTripFacade getFacade() {
        return ejbFacade;
    }
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(this.itemsPerPage) {
                ...
                public int getItemsCount() {
                    ...
                    //filter.setDayTypeFilter(filterController.getSelectedDayType());
                    filter.setDayTypeFilter(selectedDayType);
                    if (selectedArea != null && selectedArea.getTTObjId() != 0) {
                        filter.setAreaFilter(selectedArea);

                        //return getFacade().count(filter);
                    }
                    //if (selectedBranch != null && selectedBranch.getTTObjId() != 0) {
                    //filter.setBranchFilter(selectedStation);
    ...
    public String prepareView() {
        current = (PlannedTrip) getItems().getRowData();
    ...
    public String activateEdit(PlannedTrip plannedTrip) {
        current = plannedTrip;
    ...
    public String save(PlannedTrip plannedTrip) {
        try {
    ...
    public String cancel(PlannedTrip plannedTrip) {
        try {
            plannedTrip.setEditing(false);
            recreateModel();
            return "List";
    ...
    /*public DataModel getStationItems() {
     ...
     PlannedTrip trip = (PlannedTrip) item;
    ...
    /*public Date calcActionStartHour(TripAction action) {
     ...
     public Date calcActionStartHour(PlannedTrip plannedTrip, TripAction action) {
     ...
     public Date calcActionEndHour(PlannedTrip plannedTrip, TripAction action) {
